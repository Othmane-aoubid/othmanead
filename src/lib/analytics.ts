...
import { trackEvent as trackAIEvent, trackException as trackAIException } from './application-insights';
...
export function trackContactSubmit(visitorType?: string) {
  ...
  if (visitorType) {
    ...
    trackAIEvent("contact_form_submit", { visitor_type: visitorType });
    return;
  }
  sendGAEvent("event", "contact_form_submit");
  trackAIEvent("contact_form_submit");
}
...
export function trackProjectCardClick(projectId: string, projectTitle: string) {
  ...
  trackAIEvent("project_card_click", { project_id: projectId, project_title: projectTitle });
}
...
export function trackProjectLinkClick(projectId: string, linkType: "github" | "kaggle" | "details") {
  ...
  trackAIEvent("project_link_click", { project_id: projectId, link_type: linkType });
}

export function trackViewAllProjects() {
  if (!isTrackingEnabled()) return;
  sendGAEvent("event", "view_all_projects");
  trackAIEvent("view_all_projects");
}
...
export function trackBlogPostViewed(postTitle: string, postCategory: string) {
  ...
  trackAIEvent("blog_post_viewed", { post_title: postTitle, post_category: postCategory });
}
...
export function trackBlogPostClick(postId: string, postTitle: string) {
  ...
  trackAIEvent("blog_post_click", { post_id: postId, post_title: postTitle });
}

export function trackQuizStarted(quizName: string) {
  if (!isTrackingEnabled()) return;
  sendGAEvent("event", "quiz_started", { quiz_name: quizName });
  trackAIEvent("quiz_started", { quiz_name: quizName });
}
...
export function trackCTAClick(ctaType: string) {
  ...
  trackAIEvent("cta_click", { cta_type: ctaType });
}
...
export function trackLanguageSwitch(fromLang: string, toLang: string) {
  ...
  trackAIEvent("language_switch", { from_language: fromLang, to_language: toLang });
}
...
export function trackThemeSwitch(fromTheme: string, toTheme: string) {
  ...
  trackAIEvent("theme_switch", { from_theme: fromTheme, to_theme: toTheme });
}

export function trackPageScroll(scrollPercentage: number) {
  if (!isTrackingEnabled()) return;
  sendGAEvent("event", "page_scroll", { scroll_percentage: scrollPercentage });
  trackAIEvent("page_scroll", { scroll_percentage: scrollPercentage.toString() });
}

export function trackError(error: Error, context?: string) {
  if (!isTrackingEnabled()) return;
  trackAIException(error);
  trackAIEvent("error_occurred", { context: context || 'unknown', message: error.message });
}
